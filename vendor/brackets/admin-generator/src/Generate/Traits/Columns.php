...
trait Columns {
    ...
    protected function readColumnsFromTable($tableName) {
        ...
        $indexes = collect(DB::connection("sqlsrv")->getDoctrineSchemaManager()->listTableIndexes($tableName));
        return collect(DB::connection("sqlsrv")
            ->getSchemaBuilder()->getColumnListing($tableName))->map(function($columnName) use ($tableName, $indexes) {
            ...
            return [
                'name' => $columnName,
                'type' => DB::connection("sqlsrv")->getSchemaBuilder()->getColumnType($tableName, $columnName),
                'required' => boolval(DB::connection("sqlsrv")->getDoctrineColumn($tableName, $columnName)->getNotnull()),
